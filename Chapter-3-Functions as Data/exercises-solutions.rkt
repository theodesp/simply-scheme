...
(define (first-choice possibilities preferences)
  (first-if-any (keep (lambda (square) (member? square possibilities))
	       preferences)))
...
(define (tie-game? triples)
  (if (eqv? (best-free-square triples) #f) #t #f))


(define (tie-game? triples me)
  (if (and (last-choice? triples) (tied-row? (find-last-choice me))) #t #f)

(define (tied-row? triple me)
   (>= (appearances (opponent me) triple) 1))

(define (count-digits sent)
  (count (keep number? sent)))

(define (last-choice? triples)
 (<= (apply + (every (lambda (triple) (count-digits triple)) triples)) 3))
